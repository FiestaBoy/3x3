...
export async function createUser(user: FormFields): Promise<CreateUserResponse> {
  try {
    const salt = await bcrypt.genSalt(10)
    const hash = await bcrypt.hash(user.password, salt)

    if (await emailInUse(user.email)) {
      return { success: false, field: "email", message: "Email already in use"}
    }

    const statement = `INSERT INTO users (email, password_hash, birthday, first_name, last_name) VALUES (?, ?, ?, ?, ?);`

    const response = await db.query(statement, [user.email,
      hash,
      user.birthday,
      user.firstName,
      user.lastName])

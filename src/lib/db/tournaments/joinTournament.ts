...
export async function joinTournament(joinCode: string, teamId: string) {
  ...
  try {
    const teamSqlStatement =
      "SELECT captain_id, age_group FROM teams WHERE team_id = ?";
    ...
    const tournamentSqlFetchStatement =
      "SELECT tournament_id, age_group, max_teams, registration_start, registration_end FROM tournaments WHERE join_code = ?";
    const tournamentSqlFetchResponse = await db.query(
      tournamentSqlFetchStatement,
      [joinCode],
    );

    if (
      !tournamentSqlFetchResponse ||
      tournamentSqlFetchResponse.length === 0
    ) {
      throw new Error("Failed to join the tournament");
    }

    const existsSql =
      "SELECT 1 FROM team_tournament WHERE tournament_id = ? AND team_id = ? LIMIT 1";
    const existsResp = await db.query(existsSql, [
      tournamentSqlFetchResponse[0].tournament_id,
      teamId,
    ]);
    ...
    const teamCountSqlStatement =
      "SELECT COUNT(*) as count FROM team_tournament WHERE tournament_id = ?";
    const teamCountSqlResponse = await db.query(teamCountSqlStatement, [
      tournamentSqlFetchResponse[0].tournament_id,
    ]);
